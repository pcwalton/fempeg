...
import i32::range;
import io::println;
import result::{unwrap, unwrap_err};
import vector::view;

// Simple typedefs

type ustring = ~str;
type MP2Result<T> = Result<T,ustring>;
...
// Sample rate table
fn SAMPLE_RATES() -> [i32]/4 {
    [ 44100, 48000, 32000, 0 ]/_
}

// Bitrate table
fn BITRATES() -> [i32]/14 {
    [ 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384 ]/_
}
...
// Sad workaround for the lack of constant vectors in Rust.
struct MP2Constants {
    SAMPLE_RATES: [i32]/4;
    BITRATES: [i32]/14;
    SCF_VALUE: [i32]/64;
...
fn MP2Stream(context: &MP2Context) -> MP2Stream {
    fn zeroes(n: uint) -> ~[i32] {
        vector::from_elem(n, 0)
    }

    MP2Stream {
        context: context,
        V: [ zeroes(1024), zeroes(1024) ]/_,
        Voffs: 0,
        U: zeroes(512)
    }
}
...
impl MP2Stream {
    ...
    fn get_sample_rate(frame: &[u8]) -> MP2Result<i32> {
        ...
        return OK(self.context.constants.SAMPLE_RATES[(frame[2] >> 2) & 3]);
    }

    fn decode_frame(frame: &[u8], pcm: &[mut i16]) -> MP2Result<()> {
        ...
        let bitstream = Bitstream {
            ...
            bits_in_window: 8,
            frame_pos: view(frame, 3, frame.len() - 3)
        };
        ...
        let mut frame_size = 144000 * self.context.constants.BITRATES[bit_rate_index_minus1];
        frame_size /= self.context.constants.SAMPLE_RATES[sampling_frequency];
        ...
        // FIXME: Could really use repeat syntax here...
        let allocation = [
            [ mut
        ...
        // Read scale factors.
        // FIXME: Again, again...
        let triple = [ mut 0, 0, 0 ]/_;
        let scalefactor = [
            [
                copy triple, copy triple, copy triple, copy triple,
                copy triple, copy triple, copy triple, copy triple,
                copy triple, copy triple, copy triple, copy triple,
                copy triple, copy triple, copy triple, copy triple,
                copy triple, copy triple, copy triple, copy triple,
        ...
        for 3.timesi |part| {       // For each part...
            for 4.timesi |gr| {     // For each granule...
                // Read the samples.
                for bound.timesi |sb| {
                    for 2.timesi |ch| {
                        //self.read_samples(allocation[ch][sb], scalefactor[ch][sb][part], &sample[ch][sb][0])
                    }
                }
            }
        }
...
fn main(args: ~[ustring]) {
    ...
    let pcm = vector::to_mut(vector::from_elem(1000000, 0));
    let result = stream.decode_frame(bytes, pcm);
    match result {
        OK(_)    => {} // TODO
        Error(_) => { println(unwrap_err(result)); return; }
    }
}
